...
export default async function Page({ params }: PageProps<'/pages/[slug]'>) {
  const { slug } = await params
    ...
    <SitePage>
      ...
      <SitePageArticleProvider toc={post.data.toc}>
        <SitePageContent>
          {/* Render Page Article */}
          <SitePageArticle>
            <SitePageArticleContainer className="lg:grid-cols-1">
              <SitePageArticleContent className="lg:border-x">
                {/* Render Page Content */}
