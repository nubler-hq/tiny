...
<Tools>
  ...
  <ToolGroup name="Next.js Runtime Intelligence">
    ...
    <Tool name="get_errors" description="Gets real-time errors from Next.js dev server (runtime, console, build errors with source maps). USE THIS FIRST before any debugging or implementation involving existing pages." />
...
<Documents>
  <Document>
    <Path>@.rulesync/rules/auth.md</Path>
    <Description>Authentication and Authorization System</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/billing.md</Path>
    <Description>Billing and Subscription Management System Guide for SaaS Boilerplate</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/browser.md</Path>
    <Description>Browser Tools: Comprehensive Training &amp; Best Practices</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/data-table.md</Path>
    <Description>Data Table Component Usage Guide</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/docs.md</Path>
    <Description>Mandatory TSDoc Documentation Rule</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/docs_articles.md</Path>
  </Document>
  <Document>
    <Path>@.rulesync/rules/docs_updates.md</Path>
  </Document>
  <Document>
    <Path>@.rulesync/rules/email.md</Path>
    <Description>How to Create and Register Email Templates in SaaS Boilerplate</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/form.md</Path>
    <Description>How to Building Forms on SaaS Boilerplate</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/frontend.md</Path>
    <Description>Front-End Development Workflow</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/igniter-advanced-features.md</Path>
    <Description>How to Use Advanced Features of Igniter.js(Like jobs, pub/sub and more)</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/igniter-client.md</Path>
    <Description>How to use Igniter.js Client</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/igniter-controllers.md</Path>
    <Description>How create controllers on Igniter.js</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/igniter-patterns.md</Path>
    <Description>Igniter.js Controller &amp; API Development Standards</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/igniter-procedures.md</Path>
    <Description>How to Create Procedures on Igniter.js</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/notifications.md</Path>
    <Description>How to Use Notification System</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/og-image.md</Path>
    <Description>How generate dynamic OG Images</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/page.md</Path>
    <Description>How to create new pages on Dashboard</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/plugin-manager.md</Path>
    <Description>How to use Plugin Manager on SaaS Boilerplate.</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/seo.md</Path>
    <Description>How to use SEO components</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/testing.md</Path>
    <Description>How to validate API endpoints with (`get_openapi_spec` and `make_api_request`)</Description>
  </Document>
  <Document>
    <Path>@.rulesync/rules/tools.md</Path>
    <Description>How to use custom tools</Description>
  </Document>
</Documents>
...
1.  **Research and Analysis Phase**: Upon receiving a request, DO NOT start implementation. First, investigate:
    - **Health Check (CRITICAL - DO THIS FIRST)**: 
      - Execute `get_errors` to check for existing runtime errors in the Next.js dev server
      - If errors are found, analyze and determine if they're related to the current task
      - Document any pre-existing errors to avoid false assumptions about new code
    ...
    - **Current Code Verification**: Use MCP tools to inspect the real project state. For example:
      - For **page-related tasks**, always use `get_page_metadata` to understand the complete rendering hierarchy (layouts, boundaries, etc.)
...
4.  **Testing and Validation Phase**: DO NOT write unit tests. Validate your implementations using:
    - **Next.js Runtime Validation (MANDATORY)**:
      - Execute `get_errors` immediately after implementation to detect new errors
      - Compare with pre-implementation error state (from Phase 1)
      - If new errors appear, investigate using `get_logs` and `get_page_metadata`
      - Ensure zero new runtime errors before marking task as complete
    - **Page Metadata Verification** (for route/component changes):
      - Use `get_page_metadata` to confirm all expected files are rendering
      - Validate layout hierarchy is correct
...
## 9.1. NEXT.JS DIAGNOSTIC WORKFLOW

When working with Next.js applications, follow this enhanced diagnostic workflow:

### Pre-Implementation Checklist
1. **Error Baseline**: Run `get_errors` to capture current error state
...
1. **Error Regression Check**: Run `get_errors` again and compare with baseline
2. **Route Verification**: For page changes, re-run `get_page_metadata` to confirm structure
3. **Browser E2E**: Use `browser_navigate` + `browser_snapshot` for visual validation
4. **API Testing**: Use `make_api_request` for endpoint validation

### Error Response Protocol
When errors are detected via `get_errors`:
1. Analyze the full stack trace (source-mapped)
2. Identify the error type (runtime, console, build)
3. Use `analyze_file` on the files mentioned in stack traces
4. Cross-reference with `get_page_metadata` if it's a rendering issue
5. Fix the error using appropriate tools
6. Re-validate with `get_errors` to confirm resolution
